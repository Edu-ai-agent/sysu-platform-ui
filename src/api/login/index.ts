...
export const loginByMobile = (mobile: any, code: any) => {
  ...
  const scope = 'server'
  const basicAuth =
    'Basic ' + window.btoa(import.meta.env.VITE_OAUTH2_MOBILE_CLIENT)
  Session.set('basicAuth', basicAuth)
...
export const loginBySysu = (code: string, state: string) => {
  const grant_type = 'sysu'
  const scope = 'server'
  const basicAuth =
    'Basic ' + window.btoa(import.meta.env.VITE_OAUTH2_PASSWORD_CLIENT)
  Session.set('basicAuth', basicAuth)
  ...
  return request({
    ...
    method: 'post',
    params: { code, state, grant_type, scope },
  })
}
